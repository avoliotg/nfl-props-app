...
def price(sub, params, line_source, slevel=None, sigma_mode="flat",
          sigma_scale=1.0, no_model=False):
    ...
    # DECOMPOSITION NOTE. The blended mean is anchored on the CONSENSUS line
    # and the bet settles at line_source, so the total distance driving the
    # edge is
    #
    ...
    #
    # With --line-source fanduel both terms are live at once: the model's
    # disagreement with the market, and FanDuel's disagreement with the
    # market. That is why the +0.1475 result cannot be attributed without
    # separating them. --no-model zeroes the first term, --same-line removes
    # the second.
    if no_model:
        s["beta"] = 0.0
    s["blend"] = (s["line_consensus"] + s["alpha"]
                  + s["beta"] * (s["projection"] - s["line_consensus"]))
    s["dev_model"] = s["beta"] * (s["projection"] - s["line_consensus"])
    s["dev_book"] = s["line_consensus"] - s[col]
...
def main():
    ...
    ap.add_argument("--no-model", action="store_true",
                    help="force beta to 0, so the ONLY source of edge is the "
                         "bet line differing from consensus. Isolates the "
                         "stale-line effect with no model contribution.")
    ap.add_argument("--same-line", action="store_true",
                    help="keep only props where the bet line EQUALS the "
                         "consensus line. Removes the stale-line effect and "
                         "isolates the model contribution.")
    ...
    print("=" * 96)
    print("EDGE THRESHOLD v5: where a shrunk edge clears the vig")
    print(f"  betting at line_{args.line_source}")
    if args.no_model:
        print("  --no-model: beta forced to 0, STALE-LINE EFFECT ONLY")
    if args.same_line:
        print("  --same-line: bet line must equal consensus, MODEL ONLY")
    if args.line_value:
    ...
    if args.same_line:
        col = f"line_{args.line_source}"
        before = len(joined)
        joined = joined[np.isclose(joined[col], joined["line_consensus"])].copy()
        print(f"  same-line filter: {len(joined)} of {before} rows where "
              f"line_{args.line_source} equals consensus")
        if len(joined) < 300:
            print("  too few rows after the same-line filter")
            return
    ...
    s = price(joined, params, args.line_source, slevel, mode,
              args.sigma_scale, no_model=args.no_model)
    print(f"\n  EDGE DECOMPOSITION, mean absolute contribution in units of "
          f"the outcome")
    print(f"    alpha                        {s['alpha'].abs().mean():.3f}")
    print(f"    MODEL  beta*(proj-consensus) {s['dev_model'].abs().mean():.3f}"
          f"{'   ZEROED by --no-model' if args.no_model else ''}")
    print(f"    BOOK   consensus-bet line    {s['dev_book'].abs().mean():.3f}"
          f"{'   ZEROED by --same-line' if args.same_line else ''}")
    nz = int((~np.isclose(s['dev_book'], 0)).sum())
    print(f"    props where the bet line differs from consensus: "
          f"{nz} of {len(s)} ({100.0 * nz / max(len(s), 1):.1f}%)")
